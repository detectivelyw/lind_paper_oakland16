...
Google's Native Client \cite{NaCl-09} adopts SFI to execute untrusted x86 binary code safely. The difference is that SFI doesn't handle the manner in which systems interact with the underlying OS. In NaCl, to perform system calls, it relies on a trusted service runtime for thread creation, memory management, and other system services. 
