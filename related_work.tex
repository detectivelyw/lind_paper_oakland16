...
Software Fault Isolation (SFI) is a sandboxing technique in which native instructions can only be executed if they do not violate the sandbox's constraints \cite{SFI:93}. Instructions usually exclude any directions that could jump outside the program's predefined memory region or execute a system call.
